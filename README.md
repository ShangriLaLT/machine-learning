# Table of Contents
        ...
        * [<a href="#第一段的锚点名称">第一段标题</a>](#a-href第一段的锚点名称第一段标题a)
  * [一、[线性回归](/LinearRegression)](#一、[线性回归]linearregression)
    * [1、代价函数](#1、代价函数)
* [计算代价函数](#计算代价函数)
    * [2、梯度下降算法](#2、梯度下降算法)
* [梯度下降算法](#梯度下降算法)
    * [3、均值归一化](#3、均值归一化)
* [归一化feature](#归一化feature)
            * [](#)
    * [4、最终运行结果](#4、最终运行结果)
    * [5、[使用scikit-learn库中的线性模型实现](/LinearRegression/LinearRegression_scikit-learn.py)](#5、[使用scikit-learn库中的线性模型实现]linearregressionlinearregression_scikit-learnpy)
* [归一化操作](#归一化操作)
* [线性模型拟合](#线性模型拟合)
        * [](#-1)
  * [二、[逻辑回归](/LogisticRegression)](#二、[逻辑回归]logisticregression)
    * [1、代价函数](#1、代价函数-1)
    * [2、梯度](#2、梯度)
    * [3、正则化](#3、正则化)
* [代价函数](#代价函数)
* [计算梯度](#计算梯度)
    * [4、S型函数（即![{{h_\theta }(x)}](http://chart.apis.google.com/chart?cht=tx&chs=1x0&chf=bg,s,FFFFFF00&chco=000000&chl=%7B%7Bh_%5Ctheta%20%7D%28x%29%7D)）](#4、s型函数（即[{{h_theta-}x}]httpchartapisgooglecomchartchttxchs1x0chfbgsffffff00chco000000chl7b7bh_5ctheta207d28x297d）)
* [S型函数](#s型函数)
    * [5、映射为多项式](#5、映射为多项式)
* [映射为多项式](#映射为多项式)
    * [6、使用`scipy`的优化方法](#6、使用`scipy`的优化方法)
    * [7、运行结果](#7、运行结果)
    * [8、[使用scikit-learn库中的逻辑回归模型实现](/LogisticRegression/LogisticRegression_scikit-learn.py)](#8、[使用scikit-learn库中的逻辑回归模型实现]logisticregressionlogisticregression_scikit-learnpy)
* [划分为训练集和测试集](#划分为训练集和测试集)
* [归一化](#归一化)
        * [](#-2)
* [预测](#预测)
  * [[逻辑回归_手写数字识别_OneVsAll](/LogisticRegression)](#[逻辑回归_手写数字识别_onevsall]logisticregression)
    * [1、随机显示100个数字](#1、随机显示100个数字)
* [显示100个数字](#显示100个数字)
    * [2、OneVsAll](#2、onevsall)
    * [3、手写数字识别](#3、手写数字识别)
* [求每个分类的theta，最后返回所有的all_theta](#求每个分类的theta，最后返回所有的all_theta)
* [初始化变量](#初始化变量)
* [映射y](#映射y)
                                                  * [class_y[0:600,:], delimiter=',')](#class_y[0600]-delimiter)
    * [4、预测](#4、预测)
* [预测](#预测-1)
    * [5、运行结果](#5、运行结果)
    * [6、[使用scikit-learn库中的逻辑回归模型实现](/LogisticRegression/LogisticRegression_OneVsAll_scikit-learn.py)](#6、[使用scikit-learn库中的逻辑回归模型实现]logisticregressionlogisticregression_onevsall_scikit-learnpy)
  * [三、BP神经网络](#三、bp神经网络)
    * [1、神经网络model](#1、神经网络model)
    * [2、代价函数](#2、代价函数)
    * [3、正则化](#3、正则化-1)
* [代价函数](#代价函数-1)
* [还原theta1和theta2](#还原theta1和theta2)
* [np.savetxt("Theta1.csv",Theta1,delimiter=',')](#npsavetxttheta1csvtheta1delimiter)
* [映射y](#映射y-1)
* [正则化向theta^2](#正则化向theta2)
    * [4、反向传播BP](#4、反向传播bp)
* [梯度](#梯度)
* [映射y](#映射y-2)
                      * [= (h[i,:]-class_y[i,:])*sigmoidGradient(z3[i,:])  # 均方误差的误差率](#-h[i]-class_y[i]sigmoidgradientz3[i]---均方误差的误差率)
    * [5、BP可以求梯度的原因](#5、bp可以求梯度的原因)
    * [6、梯度检查](#6、梯度检查)
* [检验梯度是否计算正确](#检验梯度是否计算正确)
* [检验梯度是否计算正确](#检验梯度是否计算正确-1)
* [显示两列比较](#显示两列比较)
    * [7、权重的随机初始化](#7、权重的随机初始化)
* [随机初始化权重theta](#随机初始化权重theta)
    * [8、预测](#8、预测)
* [预测](#预测-2)
  * [= np.zeros((m,1))](#-npzerosm1)
                                                                          * [](#-3)
    * [9、输出结果](#9、输出结果)
  * [四、SVM支持向量机](#四、svm支持向量机)
    * [1、代价函数](#1、代价函数-2)
    * [2、Large Margin](#2、large-margin)
    * [3、SVM Kernel（核函数）](#3、svm-kernel（核函数）)
    * [4、使用`scikit-learn`中的`SVM`模型代码](#4、使用`scikit-learn`中的`svm`模型代码)
    * [5、运行结果](#5、运行结果-1)
  * [五、K-Means聚类算法](#五、k-means聚类算法)
    * [1、聚类过程](#1、聚类过程)
* [找到每条数据距离哪个类中心最近](#找到每条数据距离哪个类中心最近)
* [计算类中心](#计算类中心)
    * [2、目标函数](#2、目标函数)
    * [3、聚类中心的选择](#3、聚类中心的选择)
* [初始化类中心--随机取K个点作为聚类中心](#初始化类中心--随机取k个点作为聚类中心)
    * [4、聚类个数K的选择](#4、聚类个数k的选择)
    * [5、应用——图片压缩](#5、应用图片压缩)
* [聚类算法](#聚类算法)
    * [6、[使用scikit-learn库中的线性模型实现聚类](/K-Means/K-Means_scikit-learn.py)](#6、[使用scikit-learn库中的线性模型实现聚类]k-meansk-means_scikit-learnpy)
    * [7、运行结果](#7、运行结果-1)
  * [六、PCA主成分分析（降维）](#六、pca主成分分析（降维）)
    * [1、用处](#1、用处)
    * [2、2D-->1D，nD-->kD](#2、2d--1d，nd--kd)
...
* [映射数据](#映射数据)
    * [5、数据恢复](#5、数据恢复)
* [恢复数据](#恢复数据)
    * [6、主成分个数的选择（即要降的维度）](#6、主成分个数的选择（即要降的维度）)
    * [7、使用建议](#7、使用建议)
    * [8、运行结果](#8、运行结果)
    * [9、[使用scikit-learn库中的PCA实现降维](/PCA/PCA.py_scikit-learn.py)](#9、[使用scikit-learn库中的pca实现降维]pcapcapy_scikit-learnpy)
      * [coding: utf-8 -*-](#coding-utf-8---)
* [Author:bob](#authorbob)
* [Date:2016.12.22](#date20161222)
  * [七、异常检测 Anomaly Detection](#七、异常检测-anomaly-detection)
    * [1、高斯分布（正态分布）`Gaussian distribution`](#1、高斯分布（正态分布）`gaussian-distribution`)
    * [2、异常检测算法](#2、异常检测算法)
* [参数估计函数（就是求均值和方差）](#参数估计函数（就是求均值和方差）)
    * [3、评价`p(x)`的好坏，以及`ε`的选取](#3、评价`px`的好坏，以及`ε`的选取)
* [选择最优的epsilon，即：使F1Score最大](#选择最优的epsilon，即：使f1score最大)
    * [4、选择使用什么样的feature（单元高斯分布）](#4、选择使用什么样的feature（单元高斯分布）)
    * [5、多元高斯分布](#5、多元高斯分布)
* [多元高斯分布函数](#多元高斯分布函数)
    * [6、单元和多元高斯分布特点](#6、单元和多元高斯分布特点)
    * [7、程序运行结果](#7、程序运行结果)
      * [<a name="第一段的锚点名称">第一段标题</a>](#a-name第一段的锚点名称第一段标题a)
